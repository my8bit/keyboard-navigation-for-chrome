...
var migemo = {
  ...
  query: function(pattern) {
    ...
    patterns = _migemo.optimizePatterns(patterns);
...
var _migemo = {
      ...
      if (roman.slice(-1) == "c") { // special case 3
        ...
        if (roman.slice(-2) != "sh" && roman.slice(-1) != "n") { // special case 7
          ...
          for (var j = 0; j < vowel.length; j++) {
            result.push(roman.slice(0, -1) + "xtu" + c + vowel[j]);
          }
        }
  ...
  RomanToKanaDictionary: function() {
    ...
    // initialization
    this.romanPatterns = [];
    for (var p in _migemo.romanToHiraganaTable) {
      this.romanPatterns.push(p);
    }
    this.romanPatterns.sort(function(x, y) {
  ...
  extractCommonHeads: function(pattern) {
    ...
    children.sort(function(x, y) {
      return _migemo.compareStrings(x.value,
        y.value);
    });
  ...
  extractPrefix: function(pattern) {
    ...
    while (children.length > 0) {
      var currentNode = children.shift(),
        initialCharacter = currentNode.value.slice(0, 1),
        friends = [];
      while (children.length > 0) {
  ...
  Dictionary: function() {
    this.lookup = function(key) {
      var initial = key[0];
      var migemoDict = require("./migemoDict.js");
      var candidates = migemoDict[initial];
      var ans = [];
      for (var word in candidates)
        if (!word.indexOf(key)) ans = ans.concat(candidates[word]);
      return ans;
    };
  },
